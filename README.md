...
- 예를 들어 while문 안에 if문이 있으면 `들여쓰기`는 2이다.
...
- 아래 있는 `InputView`, `OutputView`, `BridgeGame`, `BridgeMaker` 클래스(또는 객체)의 요구사항을 참고하여 구현한다.
  ...
  - `BridgeGame 클래스`에서 InputView, OutputView 를 사용하지 않는다.
...
- 제공된 LottoGame 클래스를 활용해 구현해야 한다.
- LottoGame에 필드(인스턴스 변수)를 추가할 수 있다.
- LottoGame의 파일 경로는 변경할 수 있다.
- LottoGame의 메서드의 이름은 변경할 수 없고, 인자는 필요에 따라 추가하거나 변경할 수 있다.
